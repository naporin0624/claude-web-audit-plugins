...
export class LlmsTxtValidator {
    ...
    checkTitle() {
        ...
        if (this.structure.title.line !== 1) {
            ...
            if (firstNonEmptyLine + 1 !== this.structure.title.line) {
                this.issues.push(createIssue(SEVERITY.IMPORTANT, 'title-not-first', `H1 title found at line ${this.structure.title.line}, should be first`, 'Move H1 title to the beginning of the file'));
            }
        }
        this.passed.push(createPassed('title', this.structure.title.text, {
    ...
    checkSummary() {
        ...
        if (summaryLength < 20) {
            this.issues.push(createIssue(SEVERITY.RECOMMENDED, 'summary-too-short', `Summary is very short (${summaryLength} chars)`, 'Add more context to help LLMs understand your site'));
        }
        else {
            this.passed.push(createPassed('summary', `${summaryLength} characters`, {
    ...
    checkSections() {
        ...
        if (!hasOptional && !this.isFullVersion) {
            this.issues.push(createIssue(SEVERITY.RECOMMENDED, 'no-optional-section', 'No "Optional" section found', 'Add ## Optional section for less critical resources'));
        }
        this.passed.push(createPassed('sections', `${this.structure.sections.length} section(s) found`, {
    ...
    checkLinks() {
        ...
        if (invalidLinks > 0) {
            this.issues.push(createIssue(SEVERITY.IMPORTANT, 'invalid-links', `${invalidLinks} link(s) have potentially invalid URLs`, 'Use absolute URLs (https://...) or root-relative paths (/...)', { count: invalidLinks }));
        }
        if (this.structure.links.length > 0) {
            this.passed.push(createPassed('links', `${this.structure.links.length} link(s) found`));
        }
    }
    checkSize() {
        ...
        if (this.isFullVersion) {
            if (estimatedTokens > 100000) {
                this.issues.push(createIssue(SEVERITY.RECOMMENDED, 'too-large', `File may exceed typical LLM context windows (~${estimatedTokens.toLocaleString()} estimated tokens)`, 'Consider splitting into sections or prioritizing content', { estimatedTokens, sizeKB: (sizeBytes / 1024).toFixed(1) }));
            }
            else {
                this.passed.push(createPassed('size', `~${estimatedTokens.toLocaleString()} estimated tokens`, {
                    sizeKB: (sizeBytes / 1024).toFixed(1),
                }));
            }
        }
        else {
            this.passed.push(createPassed('size', `${(sizeBytes / 1024).toFixed(1)}KB`));
        }
    }
    generateReport() {
...
export function createNotFoundReport(source, isFullVersion = false) {
    ...
    return {
        ...
        issues: [
            createIssue(SEVERITY.RECOMMENDED, 'not-found', `${fileName} not found`, `Consider adding ${fileName} to improve LLM accessibility. See llmstxt.org for specification.`),
        ],
        passed: [],
    };
}
