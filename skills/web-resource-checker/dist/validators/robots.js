...
export class RobotsValidator {
    ...
    checkSize() {
        ...
        if (sizeBytes > maxBytes) {
            this.issues.push(createIssue(SEVERITY.IMPORTANT, 'file-too-large', `robots.txt is ${(sizeBytes / 1024).toFixed(1)}KB (max 500KB)`, 'Reduce file size or simplify rules', { sizeBytes, maxBytes }));
        }
        else {
    ...
    parse() {
        ...
        for (const line of lines) {
            ...
            if (colonIndex === -1) {
                this.issues.push(createIssue(SEVERITY.IMPORTANT, 'syntax-error', `Invalid syntax at line ${lineNumber}: missing colon`, 'Use format: Directive: value', { line: lineNumber, content: trimmed }));
                continue;
            }
            const directive = trimmed.substring(0, colonIndex).trim().toLowerCase();
            const value = trimmed.substring(colonIndex + 1).trim();
            if (!VALID_DIRECTIVES.includes(directive)) {
                this.issues.push(createIssue(SEVERITY.RECOMMENDED, 'unknown-directive', `Unknown directive "${directive}" at line ${lineNumber}`, `Valid directives: ${VALID_DIRECTIVES.join(', ')}`, { line: lineNumber, directive }));
                continue;
            }
    ...
    checkUserAgents() {
        ...
        if (hasWildcard) {
            ...
            this.issues.push(createIssue(SEVERITY.RECOMMENDED, 'no-wildcard-agent', 'No wildcard User-agent: * found', 'Add User-agent: * as fallback for unspecified crawlers', { agents: this.rules.map((r) => r.userAgent) }));
        }
    ...
    checkSitemap() {
        ...
        for (const sitemap of this.sitemaps) {
            if (!sitemap.url.startsWith('http://') &&
                ...
                this.issues.push(createIssue(SEVERITY.IMPORTANT, 'relative-sitemap-url', `Sitemap URL at line ${sitemap.line} is not absolute`, 'Use absolute URL: Sitemap: https://example.com/sitemap.xml', { line: sitemap.line, url: sitemap.url }));
            }
        }
        this.passed.push(createPassed('sitemap', `${this.sitemaps.length} Sitemap directive(s) found`, {
    ...
    checkBlockingAll() {
        for (const rule of this.rules) {
            if (rule.userAgent === '*' &&
                ...
                rule.allow.length === 0) {
                this.issues.push(createIssue(SEVERITY.CRITICAL, 'blocking-all', 'Disallow: / blocks all crawlers from entire site', 'Remove or modify if unintended. This prevents search engine indexing.', { userAgent: rule.userAgent, line: rule.line }));
            }
            if (rule.userAgent !== '*' &&
                rule.disallow.includes('/') &&
                rule.allow.length === 0) {
                this.passed.push(createPassed('bot-blocked', `${rule.userAgent} is blocked from site`, {
...
export function createNotFoundReport(source) {
    return {
        ...
        issues: [
            createIssue(SEVERITY.RECOMMENDED, 'not-found', 'robots.txt not found', 'Consider adding robots.txt to control crawler access'),
        ],
        passed: [],
    };
}
