...
import type { FileReport, AnalysisReport, CLIOptions, FileType } from './types.js';
import {
  SitemapValidator,
  sitemapNotFound,
  RobotsValidator,
...
interface ValidatorInstance {
  validate(): FileReport | Promise<FileReport>;
}

interface FileConfig {
  paths: string[];
  createValidator: (content: string, source: string) => ValidatorInstance;
  notFound: (source: string) => FileReport;
  async: boolean;
}

const FILES: Record<FileType, FileConfig> = {
  sitemap: {
...
export class WebResourceAnalyzer {
  ...
  private results: Record<string, FileReport> = {};
  ...
  async analyze(): Promise<AnalysisReport> {
    ...
    for (const fileKey of filesToCheck) {
      const config = FILES[fileKey as FileType];
      if (!config) continue;

      try {
        const result = await this.checkFile(fileKey as FileType, config);
  ...
  private async checkFile(fileKey: FileType, config: FileConfig): Promise<FileReport> {
  ...
  private generateReport(timestamp: string): AnalysisReport {
