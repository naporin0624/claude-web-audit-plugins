...
const STOP_WORDS = new Set([
    ...
    'must', 'can', 'this', 'that', 'these', 'those', 'it', 'its', 'you', 'your',
    'we', 'our', 'they', 'their', 'he', 'she', 'him', 'her', 'i', 'my', 'me',
    'as', 'if', 'so', 'than', 'then', 'when', 'where', 'which', 'who', 'what', 'how',
    'all', 'each', 'every', 'both', 'few', 'more', 'most', 'other', 'some', 'such',
    'no', 'not', 'only', 'same', 'just', 'also', 'very', 'about', 'into', 'through',
    'during', 'before', 'after', 'above', 'below', 'between', 'under', 'again',
    'further', 'once',
    // Japanese particles and common words
    'の', 'を', 'に', 'は', 'が', 'で', 'と', 'も', 'や', 'から', 'まで', 'より',
    'など', 'へ', 'て', 'です', 'ます', 'した', 'する', 'ある', 'いる', 'こと',
    'もの', 'これ', 'それ', 'あれ', 'この', 'その', 'あの',
]);
    ...
    analyze() {
        ...
        for (const [word, freq] of wordFreq) {
            if (wordScores.has(word)) {
                wordScores.set(word, (wordScores.get(word) || 0) + Math.min(freq, 10));
            }
        }
        // Score phrases
        ...
        for (const [phrase, freq] of phraseFreq) {
            if (phraseScores.has(phrase)) {
                phraseScores.set(phrase, (phraseScores.get(phrase) || 0) + Math.min(freq * 2, 10));
            }
            else if (freq >= 2) {
        ...
        if (topPhrases.length > 0) {
            ...
            if (!topPhrase.inTitle) {
                recommendations.push(`Consider adding "${topPhrase.phrase}" to your title tag`);
            }
            if (!topPhrase.inH1) {
                recommendations.push(`Consider including "${topPhrase.phrase}" in your H1 heading`);
            }
        }
        if (topWords.length > 0 && !topWords[0].inDescription) {
            recommendations.push(`Primary keyword "${topWords[0].word}" not found in meta description`);
        }
        return {
            file: this.filename,
