...
import type { KeywordReport, KeywordInfo, PhraseInfo, KeywordDensity } from './types.js';
...
export class KeywordAnalyzer {
  ...
  analyze(): KeywordReport {
    ...
    const topWords: KeywordInfo[] = [...wordScores.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([word, score]) => ({
        word,
    ...
    const topPhrases: PhraseInfo[] = [...phraseScores.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([phrase, score]) => ({
        phrase,
    ...
    const densities: KeywordDensity[] = topWords.slice(0, 3).map((kw) => ({
      word: kw.word,
      density:
        totalWords > 0
