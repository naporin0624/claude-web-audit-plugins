...
import type { SEOReport, KeywordReport, Severity } from './types.js';
...
export function formatSEOTextReport(report: SEOReport): string {
  ...
  const criticalIssues = report.issues.filter(
    (i) => i.severity === SEVERITY_LEVELS.CRITICAL
  );
  ...
  const importantIssues = report.issues.filter(
    (i) => i.severity === SEVERITY_LEVELS.IMPORTANT
  );
  ...
  const recommendedIssues = report.issues.filter(
    (i) => i.severity === SEVERITY_LEVELS.RECOMMENDED
  );
...
export function formatKeywordTextReport(report: KeywordReport): string {
...
export function formatCombinedTextReport(seoReport: SEOReport, keywordReport: KeywordReport): string {
